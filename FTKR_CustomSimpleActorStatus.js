 ...
 * @text SV 參戰角色動態
 * @desc 設定是否使用 SV 角色動態。
 * 0 - 不使用、1 - 常時使用、2 - 戰鬥時以外有效
 * @default 1
 * @parent sv
 * 
 * @param Sv Image Motion
 * @text SV 參戰角色標準動態
 * @desc 設定 SV 參戰角色的標準動態。
 * @default wait
 * @parent sv
 * 
 ...
 * @text 顯示升級訊息
 * @desc 設定等級提升時的訊息。
 * %1 - 角色名稱、%2 - 現在等級、%3 - 上昇等級
 * @default \C[17]%1 Level Up!
 * @parent message
 * 
 * @param customParam
 * @text --自定義參數設定--
 ...
 * @text 自定義 0 參考值
 * @desc 設定自定義 0 的參考目標。（詳細參見手冊說明）
 * 角色代碼為 a，輸入到參考目標作為指定對象。
 * @default 
 * @parent customParam
 * 
 * @param Custom 0 Unit
 ...
 * @desc 設定追加能力值的顯示名稱。
 * @type struct<xparam>
 * @default {"hit":"命中率","eva":"迴避率","cri":"爆擊率","cev":"爆擊迴避率","mev":"魔法迴避率","mrf":"魔法反射率","cnt":"反撃率","hrg":"HP再生率","mrg":"MP再生率","trg":"TP再生率"}
 * @parent paramName
 * 
 * @param SPARAM Name
 * @text 特殊能力名稱
 * @desc 設定特殊能力值的顯示名稱。
 * @type struct<sparam>
 * @default {"tgr":"目標率","grd":"防護效果","rec":"恢復效果","pha":"藥物","mcr":"MP消耗率","tcr":"TP回復率","pdr":"物理傷害","mdr":"魔法傷害","fdr":"地面傷害","exr":"經驗值"}
 * @parent paramName
 * 
 * @help
 *-----------------------------------------------------------------------------
 * 摘要
 *-----------------------------------------------------------------------------
 * 此插件將實裝可以更詳細地設定角色狀態顯示的排版。
 * 
 * 通過與該插件擴展插件（FTKR_CSS_***.js）結合使用，可以設定各種狀態畫面。
 * 例如選單畫面和戰鬥畫面。
 * 
 * 對於此插件由 v3.0.0 開始採用的每種狀態，
 * 擴展插件請使用的 v2.0.0 或更高版本作為指定顯示位置的方法。
 * ※ v1 擴展插件也可以使用常規方法。
 * 
 * 
 * 有關如何使用插件的說明，請瀏覽下面的線上手冊。
 ...
 * 1. 於「插件管理器」中，添加本插件。
 * 
 * 2. 與擴展插件（FTKR_CSS_***.js）結合使用時，請將此插件放在上方。
 * 
 * 3. 與GraphicalDesignMode.js結合使用時，請將此插件放在下方。
 * 
 * 4. 無需設定任何插件參數即可使用該插件。
 * 
 * 
